...
def sign_up_post():
    ...
    # getting and verifying proper data format
        ...
        if input_json == None: # no data recieved
             return jsn_resp.sign_up_response(403,"data format not right", raw_error="no data recieved response")
        ...
        if type(create_customer_full_json) != type({"data": "data"}):
             return jsn_resp.sign_up_response(403,"data format not right", raw_error="no keys provided, please follow " + json.dumps({"data":json_schemas.sign_up_json_resp}))
        ...
        try:
            ...
            return jsn_resp.sign_up_response(403,"data format not right", raw_error=json.dumps({'keys not like':{"data":json_schemas.sign_up_json_resp}}))

        if json_schemas.sign_up_json_resp.keys() != create_customer_full_json.keys() or create_customer_address_json.keys() != json_schemas.sign_up_json_resp['address'].keys()  or create_customer_json.keys() != json_schemas.sign_up_json_resp['customer'].keys():
            return jsn_resp.sign_up_response(403,"data format not right", raw_error=json.dumps({'keys not like':{"data":json_schemas.sign_up_json_resp}}))
        ...
        if con == False: # db con failed
            return jsn_resp.sign_up_response("400", "data recieved but connection to db failed")
            ...
            try:
                ...
                return jsn_resp.sign_up_response(403, "problem with customer address record insertion and selection",raw_error=str(e))
            ...
            try:
                ...
                db_con.destroy_connection(con) # remember to close the connection even if the execution doesnt work
                return jsn_resp.sign_up_response(403, "problem with customer record insertion and selection",raw_error=str(error))
            

            db_con.destroy_connection(con)

        return jsn_resp.sign_up_response(200,"success", data={"customer_address_record":customer_address_db_record_query_results, "customer_record": jsn_resp.sign_up_customer_response(customer_db_record_query_results[0])},user_type=0)
    except Exception as e:
        return jsn_resp.sign_up_response(403,"data format not right", raw_error=str(e))
